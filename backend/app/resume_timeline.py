...
def apply_timeline(roles, role_groups, today=None):
    ...
    restored = [role for role in forced_timeline if role["source_section"] in intervals]
    for role in roles:
    ...
    for role in restored:
        ...
        group["entries"].append(
            f"{role['employer_marker']}, {role['role_marker']} | {role['display_period']}"
            if role.get("timeline_only_due_to_thin_evidence") else
            f"{role['role_marker']} — {role['employer_marker']} | {role['display_period']}"
        )
