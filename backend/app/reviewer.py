    ...
    for item in raw.get("results") or raw.get("reviews") or []:
        ...
        for issue in item.get("issues") or []:
            ...
            if issue_type not in REVIEW_ISSUE_TYPES:
                continue
            issues.append({"type": issue_type, "description": str(issue.get("description") or "Review required.").strip()})
        status = "fail" if issues or str(item.get("status") or "").lower() == "fail" else "pass"
        results.append({
    ...
    for criteria_id in criteria_ids:
        if criteria_id not in seen:
            results.append({
                "criteria_id": criteria_id,
                "status": "fail",
                "issues": [{"type": "unsupported_claim", "description": "The Reviewer returned no decision for this criterion."}],
