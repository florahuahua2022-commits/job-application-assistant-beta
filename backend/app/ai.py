...
Flag an unqualified summary capability when the CKB supports only a narrower action or bounded scope. Flag ambiguous recency wording when "recent" does not clearly refer to the chronologically latest listed employment. Flag splitting the same skill evidence across Key Skills and Technical Skills; each source skill group must appear in one section only. Flag a generic summary opening that does not state a concrete supported role, setting, action, tool or scope.
