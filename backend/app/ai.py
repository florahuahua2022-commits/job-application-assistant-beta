...
Flag an unqualified summary capability when the CKB supports only a narrower action or bounded scope. Flag an unsupported setting named in the Summary when no selected evidence explicitly supports that setting. Flag ambiguous recency wording when "recent" does not clearly refer to the chronologically latest listed employment. Flag splitting the same skill evidence across Key Skills and Technical Skills; each source skill group must appear in one section only. Flag thin selected evidence repeated across CV sections to inflate coverage instead of being used once at its most relevant location. Flag adjacent evidence presented without explicit transferable framing or as direct experience of the advertised duty. Flag a generic summary opening that does not state a concrete supported role, setting, action, tool or scope.
