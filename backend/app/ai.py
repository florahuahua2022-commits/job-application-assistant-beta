...
- generation_under_utilized
- insufficient_source_detail
- source_parsing_uncertain
- jd_wording_repeated
- ai_tone
- declared_evidence_unused
- style_only
...
Flag wording that presents a JD-only responsibility as the applicant's experience, including project-document control, access-rights administration or named JD systems unsupported by selected CKB. A shared broad theme such as records or communication is not proof of the JD's narrower duty. Flag an unsupported causal or motivational explanation such as claiming that a role required accuracy, timeliness or a particular work driver when source_text states only the action. Flag generic recap paragraphs that repeat earlier claims without adding a selected concrete case.
...
- The closing invitation must not name or recap any role, employer, project or capability; invite further discussion without repeating the body.
- Never attach a JD-only duty such as data entry to the applicant's work unless selected CKB source_text independently supports that duty.
- Do not add any new factual claim while repairing.
- Do not infer that listed activities happened concurrently, were competing priorities or required a particular work method unless source_text says so. Factuality outranks forcing a criterion link.
...
Flag an unqualified summary capability when the CKB supports only a narrower action or bounded scope. Flag an unsupported setting named in the Summary when no selected evidence explicitly supports that setting. Flag ambiguous recency wording when "recent" does not clearly refer to the chronologically latest listed employment. A Summary may name selected earlier examples without enumerating every intervening role; wording such as "earlier roles included" does not assert a complete chronology unless it uses sequence terms such as then or next. Flag splitting the same skill evidence across Key Skills and Technical Skills; each source skill group must appear in one section only. Concise Key Skills labels may reuse supported action nouns from role evidence; flag only a repeated full or near-full duty claim, an unsupported label, or duplicated thin evidence, not shared nouns such as records, reporting or stakeholder liaison. Flag thin selected evidence repeated across CV sections to inflate coverage instead of being used once at its most relevant location. Flag adjacent evidence presented without explicit transferable framing or as direct experience of the advertised duty. Flag a generic summary opening that does not state a concrete supported role, setting, action, tool or scope.
...
def generate_draft(
    ...
    if document_type == "cover_letter":
        ...
        if identity is not None:
            company = identity.get("organisation_display_name") or "Hiring organisation"
            task += " Use job_identity.organisation_display_name in the heading; if it is empty use a neutral greeting. Never output WA gov or an unconfirmed internal label. Distinguish the advertiser from the actual employer and quote only an explicitly sourced recruitment relationship."
    ...
    if provider == "deepseek":
        try:
            return _finalise_date(_deepseek_draft(full_prompt))
        except OpenAIError as error:
    ...
    try:
        return _finalise_date(_openai_draft(full_prompt))
    except RuntimeError:
