...
export function Workspace({ applicationsPage = false }: { applicationsPage?: boolean }) {
  ...
  function deleteJobInput() {
    if (!window.confirm("Delete the pasted JD and all extracted details in this form? Saved applications and documents will not be deleted.")) return;
    resetJobInput();
    setNotice("Pasted JD and extracted details deleted. You can paste a different job now.");
  }
  ...
  async function deleteDraftApplication(application: Application) {
    if (!window.confirm(`Discard the draft for ${application.position_title}? This permanently deletes the draft and all generated documents. This action cannot be undone.`)) return;
      ...
      <section className="panel" id="add-job">
        ...
        <form onSubmit={createApplication} className="formBody compactForm">
          <div className="full fullAdBox">
            ...
            <button type="button" className="secondary" onClick={deleteJobInput} disabled={adParseState === "parsing" || jobImportState === "importing"}>Delete pasted JD</button>
            ...
            {selected ? <>
              ...
              <section className={`requirementsCard ${applicationRequirements && requirementsHasUnknown(applicationRequirements) ? "needs_confirmation" : applicationRequirements?.review_status || "loading"}`} aria-live="polite">
                ...
                {requirementsLoadState === "success" && applicationRequirements && <>
                  ...
                  <details className="advancedFormatOptions" open={isEditingRequirements}>
              ...
              {documents.length ? <>
                ...
                {activeReviewer && <div className={activeReviewer.status === "pass" ? "reviewerResult pass" : "reviewerResult fail"}><strong>{draftSaveState !== "saved" ? `${labels[activeType]} awaits a check after edits are saved` : activeReviewer.status === "pass" ? `${labels[activeType]} review passed` : activeReviewer.status === "provider_failed" || activeReviewer.status === "pending" ? `${labels[activeType]} needs review` : `${labels[activeType]} review found issues`}</strong>{activeReviewer.factual_status && <p>Factual check: <b>{activeReviewer.factual_status}</b>{activeReviewer.quality_status ? <> · Writing quality: <b>{activeReviewer.quality_status}</b></> : null}</p>}{activeReviewer.status === "provider_failed" && <p>The draft is saved. Retry its automatic review before checking the application.</p>}{activeReviewer.status === "fail" && <details><summary>View check details</summary><ul>{(activeReviewer.results || []).flatMap((result) => result.issues.map((issue) => <li key={`${result.criteria_id}-${issue.type}`}>{issue.description}{issue.location && <blockquote>{issue.location}</blockquote>}{issue.recommended_action && <p>{issue.recommended_action}</p>}{issue.evidence && <details><summary>Supporting source</summary><p>{issue.evidence}</p></details>}</li>))}</ul></details>}</div>}
